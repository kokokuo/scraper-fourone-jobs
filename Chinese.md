...
首先是 Unicode 的編碼變多？ 為什麼？ 這代表著其實每個字型並非只有一個代表的 Unicode 對應碼，很多不同的編碼皆適用，只是都會有一個代碼碼，而代表碼會是第一個，例如這邊的例子 `(` 會是 `E12D`。
...
因為上述兩個參數，會隨著 CSS 反爬蟲的難度，而有不同的重要因素，幫助我們判別，後面的例子會提到，因此先記著就好。
